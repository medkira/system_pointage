...
export function AttendanceTable(): JSX.Element {
  const { employees, updateAttendance, updateWorkingHours } = useEmployeeStore()
  ...
  const handleHoursUpdate = (employeeId: string, hours: string) => {
    ...
    if (!isNaN(numHours) && numHours >= 0 && numHours <= 24) {
      updateWorkingHours(employeeId, numHours)
    }
  }
              ...
              <TableCell>
                <Switch
                  ...
                  onCheckedChange={(checked) => {
                    updateAttendance(employee.id, checked ? 'present' : 'absent')
                  }}
                />
              </TableCell>
            </TableRow>
